...
merge m:1 geo year using "$base\Bartik.dta"
keep if _merge==3
drop _merge

areg remit exposure rururb hhsize age female if age>=18 & age<=65 [aw=weight], absorb(geo) vce(cluster geo) 

areg poor exposure rururb hhsize age female if age>=18 & age<=65 [aw=weight], absorb(geo) vce(cluster geo) 

areg epoor exposure rururb hhsize age female if age>=18 & age<=65 [aw=weight], absorb(geo) vce(cluster geo) 

ivregress 2sls poor (remit=exposure) rururb hhsize age female if age>=18 & age<=65 [aw=weight], absorb(geo) vce(cluster geo) 

ivregress 2sls epoor (remit=exposure) rururb hhsize age female if age>=18 & age<=65 [aw=weight], absorb(geo) vce(cluster geo) 
/*
foreach x in 16 18 20 22{
gen treat_20`x'=0
replace treat_20`x'=exposure if year==20`x'
label var treat_20`x' "20`x'"
}

areg remit treat_20* rururb hhsize age female if age>=18 & age<=65 [aw=weight], absorb(geo) vce(cluster geo) 
