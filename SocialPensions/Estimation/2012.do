...
Todas las bases de  datos del MCS 2012 deben estar en formato *.dta

En este programa se utilizan las siguientes bases, 
renombrándolas de la siguiente forma:

Base de población: Pobla12.dta
...
Base de no monetario hogar: G_hogar.dta
Base de no monetario personas: G_person.dta

En este programa se utilizan tres tipos de archivos, los cuales 
están ubicados en las siguientes carpetas:

1) Bases originales: "C:\Pobreza 2012\MCS"
2) Bases generadas: "C:\Pobreza 2012\Bases"
3) Bitácoras: "C:\Pobreza 2012\log"
...
gl bases="C:\Users\galve\Documents\Papers\Current\Social pension program\Data\2012\Bases";
gl log="C:\Users\galve\Documents\Papers\Current\Social pension program\Data\2012\log";
...
*Año de nacimiento;
gen anac_e=.;
replace anac_e=2012-edad if edad!=.;
...
*Identificación de la población trabajadora (toda la que reporta al menos un empleo en la base de trabajos10.dta);
gen trab=1;
...
replace pea=2 if act_buscot=="1" & (edad>=16 & edad!=.);
replace pea=0 if (edad>=16 & edad!=.) & act_buscot=="" & 
(act_rento =="2" | act_pensio=="3" | act_quehac=="4" | act_estudi=="5" | 
act_discap=="6" | act_otra=="7");
...
*Identificación de la población trabajadora (toda la que reporta al menos un empleo en la base de trabajos12.dta);
gen trab=1;
label var trab "Población con al menos un empleo";
...
gen jub=.;
*******************;
replace jub=1 if trabajo_mp=="2" & act_pensio=="3";
...
destring mat_pisos, gen(icv_pisos);
recode icv_pisos (-1=.);
recode icv_pisos (0=.);
...
*Creación del ingreso monetario deflactado a pesos de agosto del 2012;
...
por los hogares a precios de agosto de 2012. Para ellos se utilizan las 
...
con los deflactores de mayo a agosto 2012 
y de dicembre de 2011 a agosto 2012, 
respectivamente, y se obtiene el promedio mensual.;

replace ing_1=(ing_1/may12)/12 if clave=="P008" | clave=="P015";
replace ing_1=(ing_1/dic11)/12 if clave=="P009" | clave=="P016";
...
scalar lp1_urb = 1125.42;
scalar lp1_rur = 800.26;
...
scalar lp2_urb = 2328.82;
scalar lp2_rur = 1489.78;
...
sort proyecto folioviv foliohog numren;		
save "$bases\65 y mas_2012.dta", replace;

use "$data\Vivienda.dta", clear;
keep proyecto folioviv tenencia;
destring tenencia, replace;
sort proyecto folioviv ;
merge m:m proyecto folioviv using "$bases\65 y mas_2012.dta";
drop _merge;
save "$bases\65 y mas_2012.dta", replace;
...
sort proyecto folioviv foliohog numren;
merge m:m proyecto folioviv foliohog numren using "$bases\65 y mas_2012.dta";
drop _merge;
save "$bases\65 y mas_2012.dta", replace;
