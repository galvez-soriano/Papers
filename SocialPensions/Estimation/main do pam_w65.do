...
esttab using "$doc\tab1w65.tex", cells(b(star fmt(%9.3f)) se(par)) ///
...
use "$data\dbase65.dta", clear
keep if year>=2012
bysort folioviv foliohog: gen treat=1 if age>=65 & age<=67
...
graph export "$doc\graph2w65.png", replace 
* Dependent variable: Income per capita
...
size(medium) place(se) nobox just(left) margin(l+4 t+2 b+2) width(75))
graph export "$doc\graph3w65.png", replace 
...
size(medium) place(se) nobox just(left) margin(l+4 t+2 b+2) width(75))
graph export "$doc\graph4w65.png", replace
...
size(medium) place(se) nobox just(left) margin(l+4 t+2 b+2) width(75))
graph export "$doc\graph5w65.png", replace 
* Dependent variable: Health   
eststo clear
...
ysc(r(-0.2 0.2)) text(-0.21 3.12 "Feb 2013", linegap(.2cm) ///
size(medium) place(se) nobox just(left) margin(l+4 t+2 b+2) width(75))
graph export "$doc\graph6w65.png", replace 
*=====================================================================*
