...
graph export "$doc\coverage.png", replace*/
*=====================================================================*
use "$data\dbase65.dta", clear
keep if year>=2012
...
/*reg l_inc treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg poor treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg epoor treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg labor treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg hwork treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg poor_health treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg weight_h treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)

reg disabil treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg gender treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg howner treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg hli treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg rural treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg educ treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg tamhogesc treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg lremitt treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg cohab treat if year==2014 & ss_dir==0 [aw=factor], vce(cluster ubica_geo)
*Preliminar DiD
reg l_inc after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg poor after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg epoor after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg labor after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg hwork after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg poor_health after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg weight_h after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)

reg disabil after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg gender after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg howner after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg hli after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg rural after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg educ after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg tamhogesc after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg lremitt after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)
reg cohab after_treat after treat if ss_dir==0 [aw=factor], vce(cluster ubica_geo)*/
*=====================================================================*
/* TABLE 1. 
DiD estimations */
*=====================================================================*
...
eststo clear
eststo: reghdfe pam after_treat after treat educ gender disabil hli ///
remitt cohab i.tam_loc if ss_dir==0 [aw=factor], absorb(state age) cluster(ubica_geo age)
* Dependent variable: Income per capita
eststo: reghdfe l_inc after_treat after treat educ gender disabil hli ///
remitt cohab i.tam_loc if ss_dir==0 [aw=factor], absorb(state age) cluster(ubica_geo age)
* Dependent variable: Poverty. Welfare and minimum welfare lines
eststo: reghdfe poor after_treat after treat educ gender disabil hli ///
remitt cohab i.tam_loc if ss_dir==0 [aw=factor], absorb(state age) cluster(ubica_geo age)
eststo: reghdfe epoor after_treat after treat educ gender disabil hli ///
remitt cohab i.tam_loc if ss_dir==0 [aw=factor], absorb(state age) cluster(ubica_geo age)
* Dependent variable: Labor
eststo: reghdfe labor after_treat after treat educ gender disabil hli ///
remitt cohab i.tam_loc if ss_dir==0 [aw=factor], absorb(state age) cluster(ubica_geo age)
eststo: reghdfe hwork after_treat after treat educ gender disabil hli ///
remitt cohab i.tam_loc if ss_dir==0 [aw=factor], absorb(state age) cluster(ubica_geo age)
* Dependent variable: Health
eststo: reghdfe poor_health after_treat after treat educ gender disabil hli ///
remitt cohab i.tam_loc if ss_dir==0 [aw=factor], absorb(state age) cluster(ubica_geo age)
eststo: reghdfe weight_h after_treat after treat educ gender disabil hli ///
remitt cohab i.tam_loc if ss_dir==0 [aw=factor], absorb(state age) cluster(ubica_geo age)
...
eststo: reghdfe l_inc after_treat after treat educ gender disabil hli cohab ///
remitt i.tam_loc if ss_dir==0 [aw=factor], absorb(state age) cluster(ubica_geo age)
eststo: reghdfe poor after_treat after treat educ gender disabil hli cohab ///
remitt i.tam_loc if ss_dir==0 [aw=factor], absorb(state age) cluster(ubica_geo age)
eststo: reghdfe epoor after_treat after treat educ gender disabil hli cohab ///
remitt i.tam_loc if ss_dir==0 [aw=factor], absorb(state age) cluster(ubica_geo age)
eststo: reghdfe labor after_treat after treat educ gender disabil hli cohab ///
