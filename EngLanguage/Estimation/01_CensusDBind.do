...
sum hlengua [fw= factor] if state==10 & age>=3
sum elengua [fw= factor] if state==19 & age>=3
sum elengua [fw= factor] if state==25 & age>=3
sum elengua [fw= factor] if state==26 & age>=3
sum elengua [fw= factor] if state==28 & age>=3
*/
...
merge m:1 geo_mun cohort using "$data/Papers/main/EngLanguage/Data/exposure_mun.dta"
drop if _merge==2
drop _merge

merge m:1 state cohort using "$data/Papers/main/EngLanguage/Data/exposure_state.dta"
drop if _merge!=3
