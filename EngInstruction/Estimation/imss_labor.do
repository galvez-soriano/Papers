...
areg imss eng_1* eng_2* h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural i.cohort i.year if ps38==1, absorb(cct) vce(cluster cct)
coefplot, vertical keep(eng_1* eng_2*) yline(0) omitted baselevels ///
...
areg lwage eng_1* eng_2* h_eng language6 math6 female n_stud rural age age2 perma n_jobs ///
n_perma t_colle t_mast i.cohort i.year i.state if ps38==1, absorb(cct) vce(cluster cct)
coefplot, vertical keep(eng_1* eng_2*) yline(0) omitted baselevels ///
xline(3.5, lstyle(grid) lpattern(dash) lcolor(red)) ///
...
areg ldist eng_1* eng_2* h_eng language6 math6 female n_stud rural age age2 perma n_jobs ///
n_perma t_colle t_mast i.cohort i.year i.state if wage!=. & ps38==1, absorb(cct) vce(cluster cct)
coefplot, vertical keep(eng_1* eng_2*) yline(0) omitted baselevels ///
xline(3.5, lstyle(grid) lpattern(dash) lcolor(red)) ///
...
areg move_state eng_1* eng_2* h_eng language6 math6 female n_stud rural age age2 perma n_jobs ///
n_perma t_colle t_mast i.cohort i.year i.state if wage!=. & ps38==1, absorb(cct) vce(cluster cct)
coefplot, vertical keep(eng_1* eng_2*) yline(0) omitted baselevels ///
xline(3.5, lstyle(grid) lpattern(dash) lcolor(red)) ///
...
*========================================================================*
/* English exposure and Economic Industries */
*========================================================================*
use "$base\dbase_18_21_final_alter.dta", clear
...
/* Full sample */
eststo clear
eststo: areg ag_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if wage!=., absorb(cct) vce(cluster cct)
eststo: areg cons_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if wage!=., absorb(cct) vce(cluster cct)
eststo: areg manu_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if wage!=., absorb(cct) vce(cluster cct)
eststo: areg serv_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if wage!=., absorb(cct) vce(cluster cct)
...
/* Low-enrollment sample */
eststo clear
eststo: areg ag_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg cons_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg manu_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg serv_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
esttab using "$doc\tab_ea_low.tex", cells(b(star fmt(%9.3f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) title(Census estimations) keep(h_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
/* Men */
eststo clear
eststo: areg ag_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==0 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg cons_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==0 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg manu_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==0 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg serv_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==0 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
esttab using "$doc\tab_ea_men.tex", cells(b(star fmt(%9.3f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) title(Census estimations) keep(h_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
/* Women */
eststo clear
eststo: areg ag_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==1 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg cons_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==1 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg manu_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==1 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg serv_ea h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==1 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
esttab using "$doc\tab_ea_women.tex", cells(b(star fmt(%9.3f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) title(Census estimations) keep(h_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
/************* Low Enrollment Sample (Gender) *************/
gen eng_female=h_eng*female
eststo clear
eststo: areg ag_ea eng_female h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg cons_ea eng_female h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg manu_ea eng_female h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg serv_ea eng_female h_eng rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
...
*========================================================================*
/* English exposure and Economic Industries by abilities */
*========================================================================*
use "$base\dbase_18_21_final_alter.dta", clear
...
/* Low-enrollment sample */
eststo clear
eststo: areg ag_ea h_eng q2_eng q3_eng q4_eng quart* rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg cons_ea h_eng q2_eng q3_eng q4_eng quart* rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg manu_ea h_eng q2_eng q3_eng q4_eng quart* rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg serv_ea h_eng q2_eng q3_eng q4_eng quart* rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
esttab using "$doc\tab_ea_low_abil.tex", cells(b(star fmt(%9.3f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) title(Census estimations) keep(h_eng q2_eng q3_eng q4_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
/* Men */
eststo clear
eststo: areg ag_ea h_eng q2_eng q3_eng q4_eng quart* rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==0 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg cons_ea h_eng q2_eng q3_eng q4_eng quart* rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==0 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg manu_ea h_eng q2_eng q3_eng q4_eng quart* rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==0 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg serv_ea h_eng q2_eng q3_eng q4_eng quart* rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==0 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
esttab using "$doc\tab_ea_men_abil.tex", cells(b(star fmt(%9.3f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) title(Census estimations) keep(h_eng q2_eng q3_eng q4_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
/* Women */
eststo clear
eststo: areg ag_ea h_eng q2_eng q3_eng q4_eng quart* rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==1 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg cons_ea h_eng q2_eng q3_eng q4_eng quart* rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==1 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg manu_ea h_eng q2_eng q3_eng q4_eng quart* rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==1 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg serv_ea h_eng q2_eng q3_eng q4_eng quart* rural female language6 math6 age age2 perma n_jobs i.state i.cohort i.year if female==1 & ps38==1 & wage!=., absorb(cct) vce(cluster cct)
esttab using "$doc\tab_ea_women_abil.tex", cells(b(star fmt(%9.3f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) title(Census estimations) keep(h_eng q2_eng q3_eng q4_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
*========================================================================*
/* Economic Industries by English intensive jobs */
*========================================================================*
use "$base\dbase_18_21_final_alter.dta", clear
drop if wage==.
...
gen h_manu=industry==1 & eng_dist_edu>=3
gen l_manu=industry==1 & eng_dist_edu<3
gen h_svc=industry==2 & eng_dist_edu>=3
gen l_svc=industry==2 & eng_dist_edu<3
/************* Full Sample *************/
eststo clear
eststo: areg h_manu h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if wage!=., absorb(cct) vce(cluster cct)
eststo: areg l_manu h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if wage!=., absorb(cct) vce(cluster cct)
eststo: areg h_svc h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if wage!=., absorb(cct) vce(cluster cct)
eststo: areg l_svc h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if wage!=., absorb(cct) vce(cluster cct)
...
eststo clear
eststo: areg h_manu h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg l_manu h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg h_svc h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
...
eststo clear
eststo: areg h_manu h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & female==0 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg l_manu h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & female==0 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg h_svc h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & female==0 & wage!=., absorb(cct) vce(cluster cct)
...
eststo clear
eststo: areg h_manu h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & female==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg l_manu h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & female==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg h_svc h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & female==1 & wage!=., absorb(cct) vce(cluster cct)
...
gen eng_female=h_eng*female
eststo clear
eststo: areg h_manu eng_female h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg l_manu eng_female h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg h_svc eng_female h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg l_svc eng_female h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
esttab using "$doc\tab_econ_sector_low_gender.tex", ar2 cells(b(star fmt(%9.3f)) p(par([ ]))) ///
star(* 0.10 ** 0.05 *** 0.01) title(Labor Market Outcomes) keep(eng_female) replace
*========================================================================*
/* Economic Industries and English intensive jobs by abilities */
*========================================================================*
use "$base\dbase_18_21_final_alter.dta", clear
drop if wage==.
/* Creating industry variable: export industry, non-export, hospitality
...
/************* Low Enrollment Subsample *************/
eststo clear
eststo: areg h_manu h_eng q2_eng q3_eng q4_eng quart* language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg l_manu h_eng q2_eng q3_eng q4_eng quart* language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg h_svc h_eng q2_eng q3_eng q4_eng quart* language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
eststo: areg l_svc h_eng q2_eng q3_eng q4_eng quart* language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
esttab using "$doc\tab_econ_sector_low_abil.tex", cells(b(star fmt(%9.3f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) title(Labor Market Outcomes) keep(h_eng q2_eng q3_eng q4_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
/************* Low Enrollment Subsample (Men) *************/
eststo clear
eststo: areg h_manu h_eng q2_eng q3_eng q4_eng quart* language6 math6 female n_stud age age2 t_colle ///
...
eststo: areg l_svc h_eng q2_eng q3_eng q4_eng quart* language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & female==0 & wage!=., absorb(cct) vce(cluster cct)
esttab using "$doc\tab_econ_sector_low_men_abil.tex", cells(b(star fmt(%9.3f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) title(Labor Market Outcomes) keep(h_eng q2_eng q3_eng q4_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
/************* Low Enrollment Subsample (Women) *************/
eststo clear
eststo: areg h_manu h_eng q2_eng q3_eng q4_eng quart* language6 math6 female n_stud age age2 t_colle ///
...
eststo: areg l_svc h_eng q2_eng q3_eng q4_eng quart* language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & female==1 & wage!=., absorb(cct) vce(cluster cct)
esttab using "$doc\tab_econ_sector_low_women_abil.tex", cells(b(star fmt(%9.3f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) title(Labor Market Outcomes) keep(h_eng q2_eng q3_eng q4_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
*========================================================================* =================================================>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
/* Export and non-export industries */
*========================================================================*
use "$base\dbase_18_21_final_alter.dta", clear
drop if wage==.
gen industry=.
...
gen export_ind=0
replace export_ind=1 if industry==1 & activity==24 | activity==45 | activity==46 ///
| activity==54 | activity==55 | activity==97 | activity==102 | activity==106 ///
| (activity>=127 & activity<=130) | (activity>=133 & activity<=136)
gen nexport_ind=0
replace nexport_ind=1 if industry==1 & export_ind==0
/************* Full Sample *************/
eststo clear
...
t_mast rural perma n_jobs i.cohort i.year, absorb(cct) vce(cluster cct)
esttab using "$doc\tab_x_industry.tex", cells(b(star fmt(%9.3f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) title(Labor Market Outcomes) keep(h_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
/************* Low Enrollment Subsample *************/
eststo clear
eststo: areg export_ind h_eng language6 math6 female n_stud age age2 t_colle ///
...
eststo: areg nexport_ind h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1, absorb(cct) vce(cluster cct)
esttab using "$doc\tab_x_industry_low.tex", cells(b(star fmt(%9.3f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) title(Labor Market Outcomes) keep(h_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
/************* Low Enrollment Subsample (Men) *************/
eststo clear
eststo: areg export_ind h_eng language6 math6 female n_stud age age2 t_colle ///
...
eststo: areg nexport_ind h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & female==0, absorb(cct) vce(cluster cct)
esttab using "$doc\tab_x_industry_low_men.tex", cells(b(star fmt(%9.3f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) title(Labor Market Outcomes) keep(h_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
/************* Low Enrollment Subsample (Women) *************/
eststo clear
eststo: areg export_ind h_eng language6 math6 female n_stud age age2 t_colle ///
...
eststo: areg nexport_ind h_eng language6 math6 female n_stud age age2 t_colle ///
t_mast rural perma n_jobs i.cohort i.year if ps38==1 & female==1, absorb(cct) vce(cluster cct)
esttab using "$doc\tab_x_industry_low_women.tex", cells(b(star fmt(%9.3f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) title(Labor Market Outcomes) keep(h_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
/************* Low Enrollment Subsample (Gender) *************/
gen eng_female=h_eng*female
eststo clear
...
t_mast rural perma n_jobs i.cohort i.year if ps38==1, absorb(cct) vce(cluster cct)
esttab using "$doc\tab_x_industry_low_gender.tex", ar2 cells(b(star fmt(%9.3f)) p(par([ ]))) ///
star(* 0.10 ** 0.05 *** 0.01) title(Labor Market Outcomes) keep(eng_female) replace
*========================================================================*
...
/************* Full ENLACE Sample *************/
eststo clear
eststo: areg language6 h_eng language5 female n_stud t_colle t_mast rural i.cohort i.year, absorb(state_s) vce(cluster cct)
eststo: areg language6 h_eng language5 female n_stud t_colle t_mast rural i.cohort i.year, absorb(cct) vce(cluster cct)
eststo: areg math6 h_eng math5 female n_stud t_colle t_mast rural i.cohort i.year, absorb(state_s) vce(cluster cct)
eststo: areg math6 h_eng math5 female n_stud t_colle t_mast rural i.cohort i.year, absorb(cct) vce(cluster cct)
...
/************* Full Sample *************/
eststo clear
eststo: areg language6 h_eng language5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=., absorb(state_s) vce(cluster cct)
eststo: areg language6 h_eng language5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=., absorb(cct) vce(cluster cct)
eststo: areg math6 h_eng math5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=., absorb(state_s) vce(cluster cct)
eststo: areg math6 h_eng math5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=., absorb(cct) vce(cluster cct)
esttab using "$doc\tab_stud.tex", cells(b(star fmt(%9.4f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) keep(h_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
/************* Low Enrollment Sample *************/
eststo clear
eststo: areg language6 h_eng language5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=. & ps38==1, absorb(state_s) vce(cluster cct)
eststo: areg language6 h_eng language5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=. & ps38==1, absorb(cct) vce(cluster cct)
eststo: areg math6 h_eng math5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=. & ps38==1, absorb(state_s) vce(cluster cct)
eststo: areg math6 h_eng math5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=. & ps38==1, absorb(cct) vce(cluster cct)
esttab using "$doc\tab_stud_low.tex", cells(b(star fmt(%9.4f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) keep(h_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
/************* Low Enrollment Sample (Men) *************/
eststo clear
eststo: areg language6 h_eng language5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=. & ps38==1 & female==0, absorb(state_s) vce(cluster cct)
eststo: areg language6 h_eng language5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=. & ps38==1 & female==0, absorb(cct) vce(cluster cct)
eststo: areg math6 h_eng math5 female n_stud t_colle t_mast i.cohort rural i.year if wage!=. & ps38==1 & female==0, absorb(state_s) vce(cluster cct)
eststo: areg math6 h_eng math5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=. & ps38==1 & female==0, absorb(cct) vce(cluster cct)
esttab using "$doc\tab_stud_low_men.tex", cells(b(star fmt(%9.4f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) keep(h_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
/************* Low Enrollment Sample (Women) *************/
eststo clear
eststo: areg language6 h_eng language5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=. & ps38==1 & female==1, absorb(state_s) vce(cluster cct)
eststo: areg language6 h_eng language5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=. & ps38==1 & female==1, absorb(cct) vce(cluster cct)
eststo: areg math6 h_eng math5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=. & ps38==1 & female==1, absorb(state_s) vce(cluster cct)
eststo: areg math6 h_eng math5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=. & ps38==1 & female==1, absorb(cct) vce(cluster cct)
esttab using "$doc\tab_stud_low_women.tex", cells(b(star fmt(%9.4f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) keep(h_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace
/************* Low Enrollment Sample (Gender) *************/
gen eng_female=h_eng*female
eststo clear
eststo: areg language6 eng_female h_eng language5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=. & ps38==1, absorb(state_s) vce(cluster cct)
eststo: areg language6 eng_female h_eng language5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=. & ps38==1, absorb(cct) vce(cluster cct)
eststo: areg math6 eng_female h_eng math5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=. & ps38==1, absorb(state_s) vce(cluster cct)
eststo: areg math6 eng_female h_eng math5 female n_stud t_colle t_mast rural i.cohort i.year if wage!=. & ps38==1, absorb(cct) vce(cluster cct)
...
n_perma t_colle t_mast i.cohort i.year i.state if wage!=., absorb(cct) vce(cluster cct)
esttab using "$doc\tab_labor_lee.tex", cells(b(star fmt(%9.3f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) title(Labor Market Outcomes) keep(h_eng) stats(N ar2, fmt(%9.0fc %9.3f)) replace

areg lwage eng language6 math6 female n_stud rural age age2 perma n_jobs ///
n_perma t_colle t_mast i.cohort i.year i.state if ps38==1, absorb(cct) vce(cluster cct)

leebounds lwage eng language6 math6 female n_stud rural age age2 perma n_jobs ///
n_perma t_colle t_mast i.cohort i.year i.state if ps38==1 vce(cluster cct)
*========================================================================* =================================================>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
/******************************* Appendix ********************************/
*========================================================================*
*========================================================================*
...
eststo clear
eststo: areg lwage h_eng language6 math6 female n_stud rural age age2 perma n_jobs ///
n_perma t_colle t_mast i.cohort i.year i.state if wage!=., absorb(state_s) vce(cluster cct)
eststo: areg lwage h_eng language6 math6 female n_stud rural age age2 perma n_jobs ///
n_perma t_colle t_mast i.cohort i.year i.state if wage!=., absorb(geo_mun_s) vce(cluster cct)
eststo: areg lwage h_eng language6 math6 female n_stud rural age age2 perma n_jobs ///
n_perma t_colle t_mast i.cohort i.year i.state if wage!=., absorb(id_geo_s) vce(cluster cct)
eststo: areg lwage h_eng language6 math6 female n_stud rural age age2 perma n_jobs ///
n_perma t_colle t_mast i.cohort i.year i.state if wage!=., absorb(cct) vce(cluster cct)
...
eststo clear
eststo: areg lwage h_eng language6 math6 female n_stud rural age age2 perma n_jobs ///
n_perma t_colle t_mast i.cohort i.year i.state if ps38==1 & wage!=., absorb(state_s) vce(cluster cct)
eststo: areg lwage h_eng language6 math6 female n_stud rural age age2 perma n_jobs ///
n_perma t_colle t_mast i.cohort i.year i.state if ps38==1 & wage!=., absorb(geo_mun_s) vce(cluster cct)
eststo: areg lwage h_eng language6 math6 female n_stud rural age age2 perma n_jobs ///
n_perma t_colle t_mast i.cohort i.year i.state if ps38==1 & wage!=., absorb(id_geo_s) vce(cluster cct)
eststo: areg lwage h_eng language6 math6 female n_stud rural age age2 perma n_jobs ///
n_perma t_colle t_mast i.cohort i.year i.state if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
...
star(* 0.10 ** 0.05 *** 0.01) rename(hrs_exp h_eng hrs_exp_loc h_eng) keep(h_eng) stats(N ar2, fmt(%12.0fc %9.3f)) replace
...
esttab using "$doc\tab_exp_low_gender.tex", ar2 cells(b(star fmt(%9.3f)) p(par([ ]))) ///
