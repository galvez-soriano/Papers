...
gl base= "C:\Users\ogalvezs\Documents\English paper\Data"
gl doc= "C:\Users\ogalvezs\Documents\English paper\Doc"
*========================================================================*
...
esttab using "$doc\tab_labor_low_gender.tex", ar2 cells(b(star fmt(%9.3f)) p()) ///
...
t_mast rural i.cohort i.year if ps38==1, absorb(cct) vce(cluster cct)
esttab using "$doc\tab_econ_sector_low_gender.tex", ar2 cells(b(star fmt(%9.3f)) p()) ///
star(* 0.10 ** 0.05 *** 0.01) title(Labor Market Outcomes) keep(eng_female) replace
*========================================================================*
...
t_mast rural i.cohort i.year if ps38==1 & wage!=., absorb(cct) vce(cluster cct)
esttab using "$doc\tab_com_size_low_gender.tex", ar2 cells(b(star fmt(%9.3f)) p()) ///
star(* 0.10 ** 0.05 *** 0.01) title(Labor Market Outcomes) keep(eng_female) replace
*========================================================================*
...
/************* Low Enrollment Sample *************/
eststo clear
eststo: areg language6 h_eng language5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1, absorb(state_s) vce(cluster cct)
eststo: areg language6 h_eng language5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1, absorb(cct) vce(cluster cct)
eststo: areg math6 h_eng math5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1, absorb(state_s) vce(cluster cct)
eststo: areg math6 h_eng math5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1, absorb(cct) vce(cluster cct)
esttab using "$doc\tab_stud_low.tex", ar2 cells(b(star fmt(%9.4f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) keep(h_eng) replace
/************* Low Enrollment Sample (Men) *************/
eststo clear
eststo: areg language6 h_eng language5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1 & gender==0, absorb(state_s) vce(cluster cct)
eststo: areg language6 h_eng language5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1 & gender==0, absorb(cct) vce(cluster cct)
eststo: areg math6 h_eng math5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1 & gender==0, absorb(state_s) vce(cluster cct)
eststo: areg math6 h_eng math5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1 & gender==0, absorb(cct) vce(cluster cct)
esttab using "$doc\tab_stud_men.tex", ar2 cells(b(star fmt(%9.4f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) keep(h_eng) replace
/************* Low Enrollment Sample (Women) *************/
eststo clear
eststo: areg language6 h_eng language5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1 & gender==1, absorb(state_s) vce(cluster cct)
eststo: areg language6 h_eng language5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1 & gender==1, absorb(cct) vce(cluster cct)
eststo: areg math6 h_eng math5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1 & gender==1, absorb(state_s) vce(cluster cct)
eststo: areg math6 h_eng math5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1 & gender==1, absorb(cct) vce(cluster cct)
esttab using "$doc\tab_stud_women.tex", ar2 cells(b(star fmt(%9.4f)) se(par)) ///
star(* 0.10 ** 0.05 *** 0.01) keep(h_eng) replace
/************* Low Enrollment Sample (Gender) *************/
gen eng_female=h_eng*female
eststo clear
eststo: areg language6 eng_female h_eng language5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1, absorb(state_s) vce(cluster cct)
eststo: areg language6 eng_female h_eng language5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1, absorb(cct) vce(cluster cct)
eststo: areg math6 eng_female h_eng math5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1, absorb(state_s) vce(cluster cct)
eststo: areg math6 eng_female h_eng math5 gender n_stud t_colle t_mast rural i.cohort if wage!=. & ps38==1, absorb(cct) vce(cluster cct)
esttab using "$doc\tab_stud_low_gender.tex", ar2 cells(b(star fmt(%9.4f)) p()) ///
...
esttab using "$doc\tab_ea_low_gender.tex", ar2 cells(b(star fmt(%9.3f)) p()) ///
...
star(* 0.10 ** 0.05 *** 0.01) keep(hrs_exp hrs_exp_loc h_eng) replace
...
esttab using "$doc\tab_exp_low_gender.tex", ar2 cells(b(star fmt(%9.4f)) p()) ///
star(* 0.10 ** 0.05 *** 0.01) keep(eng_female_c eng_female_l eng_female) replace
*========================================================================*
...
esttab using "$doc\tab_labor_abil_gender.tex", ar2 cells(b(star fmt(%9.3f)) p()) ///
star(* 0.10 ** 0.05 *** 0.01) title(Labor Market Outcomes) keep(eng_female) replace
