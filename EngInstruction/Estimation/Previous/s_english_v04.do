...
table state if shift==1 & year==2008 & public==1 & fts==0, c(mean eng n eng)
table state if shift==1 & year==2011 & public==1 & fts==0, c(mean eng n eng)

table state if shift==1 & year==2008 & public==1 & fts==0 & rural==1, c(mean eng n eng)
table state if shift==1 & year==2011 & public==1 & fts==0 & rural==1, c(mean eng n eng)

table state if shift==1 & year==2008 & public==1 & fts==0 & rural==0, c(mean eng n eng)
table state if shift==1 & year==2011 & public==1 & fts==0 & rural==0, c(mean eng n eng)
*/
*========================================================================*
* Hours over time
*========================================================================*
...
keep cct year state rural hours_eng total_stud
replace hours_eng=0 if hours_eng==.
collapse (sum) total_stud (mean) hours_eng, by(year state rural cct)
reshape wide hours_eng total_stud, i(cct rural state) j(year)
...
*keep cct
*gen sw_eng=1
*save "$base\sw_eng.dta", replace
eststo clear
foreach x in 1 2 3 4 5 6 7 8 10 11 12 13 14 15 17 18 19 21 22 24 ///
25 26 27 28 29 30 31 32 {
quietly reg post_pre hours_eng2006 if state==`x' & rural==0
estimates store s`x'
}
...
*graph export "$doc\graph08u.png", replace
