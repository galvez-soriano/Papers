...
inc_hh edu_hh [aw=weight]if cohort>=1986 & cohort<=1995 & edu<=12, absorb(geo) vce(cluster geo)
...
[aw=weight]if cohort>=1986 & cohort<=1995 & edu<=12, absorb(geo) vce(cluster geo)
coefplot, vertical keep(treat_*) yline(0) omitted baselevels ///
xline(4.5, lstyle(grid) lpattern(dash) lcolor(red)) ///
