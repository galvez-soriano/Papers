...
* This program was originally run with Stata 18. The databases shoul have format *.dta
...
gl data="C:\Users\Oscar Galvez Soriano\Documents\Papers\Remittances\Data\2024\OData";
gl bases="C:\Users\Oscar Galvez Soriano\Documents\Papers\Remittances\Data\2024\Bases";
gl log="C:\Users\Oscar Galvez Soriano\Documents\Papers\Remittances\Data\2024\Log";
...
rename etnia indigen;
destring indigen, replace;
recode indigen (2=0);
...
gen time_commsvc=hor_3+(min_3/60);
replace time_commsvc=0 if time_commsvc==.;
gen time_caregiver=hor_4+(min_4/60);
replace time_caregiver=0 if time_caregiver==.;
gen time_repair=hor_5+(min_5/60);
replace time_repair=0 if time_repair==.;
gen time_hclean=hor_6+(min_6/60);
replace time_hclean=0 if time_hclean==.;
gen time_leisure=hor_8+(min_8/60);
replace time_leisure=0 if time_leisure==.;

keep folioviv foliohog numren edad anac_e student niv_ed indigen
parentesco hli educ time_work time_study time_repair time_leisure
time_commsvc time_caregiver time_hclean;
...
* recode s_salud (.=0) if missing(s_salud);
...
keep folioviv foliohog factor tam_loc rururb tamhogesc ict ictpc plp_e plp 
	 est_dis upm ubica_geo tot_integ ing_mon ing_lab ing_ren ing_tra ; 
sort folioviv foliohog;
...
order 	folioviv foliohog numren est_dis upm 
		factor tam_loc rururb ent ubica_geo edad sexo parentesco educ indigen
		time_work time_study time_repair time_leisure
		formal htrab tipo_trab* sinco scian remesas
		anac_e student 
		pea pam ing_pam
...
label define epoori   0 "Not extreme poor i" 1 "Extreme poor i";
...
label define poori   0 "Not poor i" 1 "Poor i";
label value poori poori;
*******************************************************************
*Extreme poverty (income);
gen epoor=1 if ictpc<lp1_urb & rururb==0;
replace epoor=0 if ictpc>=lp1_urb & rururb==0 & ictpc!=.;
replace epoor=1 if ictpc<lp1_rur & rururb==1;
replace epoor=0 if ictpc>=lp1_rur & rururb==1 & ictpc!=.;

label define epoor   0 "Not extreme poor" 1 "Extreme poor";
label value epoor epoor;

*Poverty (income);
gen poor=1 if (ictpc<lp2_urb & rururb==0);
replace poor=0 if (ictpc>=lp2_urb & rururb==0) & ictpc!=.;
replace poor=1 if (ictpc<lp2_rur & rururb==1);
replace poor=0 if (ictpc>=lp2_rur & rururb==1) & ictpc!=.;

label define poor   0 "Not poor" 1 "Poor";
label value poor poor;
**************************************************************************
...
gen lblepoor=plp_e==1;
label var lblepoor "Individual in extreme poverty";
drop plp_e;
gen lblpoor=plp==1;
label var lblpoor "Individual in poverty";
drop plp;
