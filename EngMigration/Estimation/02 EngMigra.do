...
*========================================================================*
/* Panel (e). Likelihood of migrating to Canada */
*========================================================================*
gen dest_can=destination_c==213
replace dest_can=. if destination_c==.

csdid dest_can female if dmigrant==0 & migrant==1 [iw=factor], ///
time(cohort) gvar(first_cohort) method(dripw) vce(cluster geo) long2 wboot seed(6)
estat event, window(-5 7) estore(csdid_CANmigrant)

coefplot csdid_CANmigrant, vertical yline(0) drop(Pre_avg Post_avg) omitted baselevels ///
xline(5.5, lstyle(grid) lpattern(dash) lcolor(red)) ///
ytitle("Likelihood of migrating to Canada", size(medium) height(5)) ///
ylabel(-1(0.5)1, labs(medium) grid format(%5.2f)) ///
xtitle("Cohorts since policy intervention", size(medium) height(5)) ///
xlabel(, angle(horizontal) labs(medium)) ///
graphregion(color(white)) scheme(s2mono) ciopts(recast(rcap)) ///
...
Tm2 = "-2" Tp0 = "0" Tp1 = "1" Tp2 = "2" Tp3 = "3" Tp4 = "4" ///
Tp5 = "5" Tp6 = "6" Tp7 = "7" Tp8 = "8")
graph export "$doc\PTA_CS_CANmigrant.png", replace



*========================================================================*

csdid migrant if dmigrant==0 & female==0 [iw=factor], ///
time(cohort) gvar(first_cohort) method(dripw) vce(cluster geo) long2 wboot seed(6)
estat event, window(-5 7) estore(csdid_migrantM)

csdid migrant if dmigrant==0 & female==1 [iw=factor], ///
time(cohort) gvar(first_cohort) method(dripw) vce(cluster geo) long2 wboot seed(6)
estat event, window(-5 7) estore(csdid_migrantW)

coefplot ///
(csdid_migrantM, label(Men) msymbol(O) mcolor(blue) ciopt(lc(blue) recast(blue)) lc(blue)) ///
(csdid_migrantW, offset(-0.1) label(Women) msymbol(T) mcolor(ltblue) ciopt(lc(ltblue) recast(rcap)) lc(ltblue)), ///
...
Tm2 = "-2" Tp0 = "0" Tp1 = "1" Tp2 = "2" Tp3 = "3" Tp4 = "4" ///
Tp5 = "5" Tp6 = "6" Tp7 = "7" Tp8 = "8")
graph export "$doc\PTA_CS_migrantSex.png", replace
